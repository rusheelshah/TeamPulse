...
//View controller to store responses to surveys from players
class PlayerResponseViewController: UIViewController {
        ...
        self.surveyQuestionsRef.observe(.value, with: { (snapshot) in
            if(snapshot.hasChildren()){
                for child in snapshot.children{
                    //Pulls questions from firebase and presents them
    ...
    //Determines interval for each answer
    ...
    @IBAction func onDoneButton(_ sender: UIBarButtonItem) {
        //If user didn't move slider, count it as neutral
        if(self.value1 == nil){
            self.value1 = 3
        }
        if(self.value2 == nil){
